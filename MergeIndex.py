...
class MergeIndex:
    ...
    def _create_category_index(self, sorted_data):
        ...
        for token, new_postings in sorted_data.items():
            ...
            # Merge postings for the current token
            if token in category_data:
                # Combine postings, avoiding duplicates
                existing_postings = set(category_data[token])
                updated_postings = list(existing_postings.union(new_postings))
            else:
                updated_postings = new_postings

            # Update the token's postings
            category_data[token] = updated_postings

            # Write the updated category data back to the JSON file
            with open(file_path, 'w') as file:
                json.dump(category_data, file, indent=4)  # Write as formatted JSON
